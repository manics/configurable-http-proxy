...
export class ConfigurableProxy extends EventEmitter {
  constructor(options) {
    ...
    var logErrors = (handler) => {
      return async function (req, res) {
        try {
          await handler.apply(that, arguments);
        } catch (e) {
  ...
  async addRoute(path, data) {
    ...
    await this._routes.add(path, data);
    await this.updateLastActivity(path);
  ...
  async removeRoute(path) {
    ...
    const result = await routes.get(path);
    if (result) {
      this.log.info("Removing route %s", path);
      await routes.remove(path);
    }
  }

  async getRoute(req, res, path) {
    // GET a single route
    path = this._routes.cleanPath(path);
    const route = await this._routes.get(path);
    if (!route) {
      res.writeHead(404);
      res.end();
      return;
  ...
  async getRoutes(req, res, path) {
    ...
    const routes = await this._routes.getAll();
    var results = {};
  ...
  async postRoutes(req, res, path, data) {
    ...
    await this.addRoute(path, data);
    res.writeHead(201);
    res.end();
    this.metrics.apiRouteAddCount.inc();
  }

  async deleteRoutes(req, res, path) {
    // DELETE removes an existing route
    const result = await this._routes.get(path);
    var code;
    if (result) {
      await this.removeRoute(path);
      code = 204;
    } else {
      code = 404;
    }
  ...
  async updateLastActivity(prefix) {
    var metricsTimerEnd = this.metrics.lastActivityUpdatingSummary.startTimer();
    var routes = this._routes;

    const result = await routes.get(prefix);
    if (result) {
      await routes.update(prefix, { last_activity: new Date() });
    }
    metricsTimerEnd();
  }
  ...
  async handleProxyError(code, kind, req, res, e) {
  ...
  async handleProxy(kind, req, res) {
    // proxy any request
    var that = this;
    ...
    try {
      const match = await this.targetForReq(req);
      if (!match) {
  ...
  async handleProxyWs(req, socket, head) {
    // Proxy a websocket request
    this.metrics.requestsWsCount.inc();
    await this.handleProxy("ws", req, socket, head);
  }
  ...
  async handleProxyWeb(req, res) {
    ...
    await this.handleProxy("web", req, res);
  }
    ...
    for (var i = 0; i < this.apiHandlers.length; i++) {
      ...
      if (match) {
        ...
        try {
          ...
          // Handle any errors that might occur during the handler execution
          this.log.error("Error in API handler: %s", e);
