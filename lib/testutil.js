...
export async function addTarget(proxy, path, port, websocket, targetPath, sslOptions) {
  ...
  server.listen(port);
  servers.push(server);
  await proxy.addRoute(path, { target: target });
  ...
  await proxy._routes.update(path, { last_activity: proxy._setup_timestamp });
}

export async function addTargetRedirecting(proxy, path, port, targetPath, redirectTo) {
...
export async function addTargets(proxy, paths, port) {
  if (paths.length === 0) {
    return;
  }
  await addTarget(proxy, paths[0], port, true, null);
  await addTargets(proxy, paths.slice(1), port + 1);
}

export async function setupProxy(port, options, paths) {
  options = options || {};
  ...
  await addTargets(proxy, paths || ["/"], port + 2);
